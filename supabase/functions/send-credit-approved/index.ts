...
interface CreditApprovedPayload {
  user_id: string;
  partner_bank: string;
...
serve(async (req) => {
  ...
  try {
    console.log('📧 Send Credit Approved email started');

    const payload: CreditApprovedPayload = await req.json();
...
<body style="margin: 0; padding: 0; background-color: #f5f5f7; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f5f7;">
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -2px rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="padding: 0 40px 40px 40px;">
              ...
              <h2 style="margin: 0 0 16px 0; color: #059669; font-size: 24px; font-weight: 700; text-align: center;">
                Kredit genehmigt!
              </h2>
              ...
              <p style="margin: 0 0 24px 0; color: #4b5563; font-size: 15px; line-height: 1.6;">
                wir freuen uns, Ihnen mitteilen zu können, dass Ihr Kreditantrag erfolgreich genehmigt wurde!
              </p>
    ...
    const { data: emailResult, error: emailError } = await resend.emails.send({
      ...
      subject: `Kredit genehmigt! - ${brandingName}`,
      html: htmlEmail,
    });
    ...
    console.log('✅ Credit approved email sent successfully:', emailResult);
    ...
    console.error('💥 Error in send-credit-approved:', error);
    return new Response(
