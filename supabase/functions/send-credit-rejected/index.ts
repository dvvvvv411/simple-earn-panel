...
interface CreditRejectedPayload {
  user_id: string;
...
serve(async (req) => {
  ...
  try {
    console.log('📧 Send Credit Rejected email started');

    const payload: CreditRejectedPayload = await req.json();
...
<head>
  ...
  <title>Kreditantrag - Aktualisierung</title>
</head>
<body style="margin: 0; padding: 0; background-color: #f5f5f7; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f5f7;">
          ...
          <tr>
            <td style="padding: 0 40px 40px 40px;">
              ...
              <p style="margin: 0 0 24px 0; color: #4b5563; font-size: 15px; line-height: 1.6;">
                leider können wir Ihren Kreditantrag zum jetzigen Zeitpunkt nicht genehmigen.
              </p>
              ...
              <p style="margin: 0 0 24px 0; color: #4b5563; font-size: 15px; line-height: 1.6;">
                Falls Sie Fragen haben oder zusätzliche Informationen einreichen möchten, wenden Sie sich bitte an unseren Support.
              </p>
    ...
    const { data: emailResult, error: emailError } = await resend.emails.send({
      ...
      subject: `Kreditantrag - Aktualisierung - ${brandingName}`,
      html: htmlEmail,
    });
    ...
    console.log('✅ Credit rejected email sent successfully:', emailResult);
    ...
    console.error('💥 Error in send-credit-rejected:', error);
    return new Response(
