...
serve(async (req) => {
  ...
  try {
    console.log('📧 Send Task Approved email started');
    ...
    const formattedCompensation = new Intl.NumberFormat('de-DE', {
      style: 'currency',
      currency: 'EUR'
    }).format(payload.compensation);
...
<body style="margin: 0; padding: 0; background-color: #f5f5f7; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f5f7;">
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="background-color: #ffffff; padding: 32px 40px; text-align: center; border-bottom: 1px solid #e5e7eb;">
              ...
              <p style="margin: 0; color: #6b7280; font-size: 14px; font-weight: 500;">Auftragsbestätigung</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 0 40px 40px 40px;">
              ...
              <p style="margin: 0 0 24px 0; color: #4b5563; font-size: 15px; line-height: 1.6;">
                Ihr Auftrag wurde erfolgreich genehmigt und die Vergütung wurde Ihrem Handelskonto gutgeschrieben.
              </p>

              <!-- Task Details -->
              <div style="background-color: #dcfce7; border: 1px solid #86efac; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
                <p style="margin: 0 0 8px 0; color: #166534; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;">
                  Auftrag
                </p>
                <p style="margin: 0 0 12px 0; color: #166534; font-size: 18px; font-weight: 600;">
                  ${payload.task_title}
                </p>
                <div style="background-color: #166534; color: white; padding: 12px 16px; border-radius: 8px; text-align: center;">
                  <p style="margin: 0 0 4px 0; font-size: 12px; opacity: 0.9;">Gutgeschrieben</p>
                  <p style="margin: 0; font-size: 28px; font-weight: 700;">${formattedCompensation}</p>
                </div>
              </div>
    ...
    const { data: emailResult, error: emailError } = await resend.emails.send({
      ...
      subject: `Auftrag genehmigt: ${payload.task_title} - ${brandingName}`,
      html: htmlEmail,
    });
    ...
    console.log('✅ Task approved email sent successfully:', emailResult);
    ...
    console.error('💥 Error in send-task-approved:', error);
    return new Response(
