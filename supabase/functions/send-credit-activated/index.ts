...
interface CreditActivatedPayload {
  user_id: string;
}
...
serve(async (req) => {
  ...
  try {
    console.log('📧 Send Credit Activated email started');

    const payload: CreditActivatedPayload = await req.json();
...
<head>
  ...
  <title>Kreditantrag - Unterlagen einreichen</title>
</head>
<body style="margin: 0; padding: 0; background-color: #f5f5f7; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f5f7;">
          ...
          <tr>
            <td style="padding: 0 40px 40px 40px;">
              ...
              <p style="margin: 0 0 24px 0; color: #4b5563; font-size: 15px; line-height: 1.6;">
                Sie können nun einen Kredit beantragen. Bitte reichen Sie Ihre Unterlagen ein, um den Antragsprozess zu starten.
              </p>
    ...
    const { data: emailResult, error: emailError } = await resend.emails.send({
      ...
      subject: `Kreditantrag - Unterlagen einreichen - ${brandingName}`,
      html: htmlEmail,
    });
    ...
    console.log('✅ Credit activated email sent successfully:', emailResult);
    ...
    console.error('💥 Error in send-credit-activated:', error);
    return new Response(
