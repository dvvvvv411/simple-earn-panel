...
serve(async (req) => {
  ...
  try {
    console.log('📧 Send Task Rejected email started');

    const payload: TaskRejectedPayload = await req.json();
...
<head>
  ...
  <title>Auftrag nicht genehmigt</title>
</head>
<body style="margin: 0; padding: 0; background-color: #f5f5f7; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f5f7;">
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="background-color: #ffffff; padding: 32px 40px; text-align: center; border-bottom: 1px solid #e5e7eb;">
              ...
              <p style="margin: 0; color: #6b7280; font-size: 14px; font-weight: 500;">Auftrags-Aktualisierung</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 40px 40px 20px 40px; text-align: center;">
              <div style="width: 80px; height: 80px; background-color: #fee2e2; border-radius: 50%; display: inline-block; line-height: 80px;">
          ...
          <tr>
            <td style="padding: 0 40px 40px 40px;">
              ...
              <p style="margin: 0 0 24px 0; color: #4b5563; font-size: 15px; line-height: 1.6;">
                Leider konnte Ihr Auftrag nicht genehmigt werden.
              </p>

              <!-- Rejection Details -->
              <div style="background-color: #fef2f2; border: 1px solid #fecaca; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
                <p style="margin: 0 0 8px 0; color: #991b1b; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;">
                  Auftrag
                </p>
                <p style="margin: 0 0 16px 0; color: #991b1b; font-size: 18px; font-weight: 600;">
                  ${payload.task_title}
                </p>
                <p style="margin: 0 0 8px 0; color: #991b1b; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;">
                  Grund
              ...
              <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                <tr>
                  <td style="text-align: center;">
                    <a href="https://${domain}/kryptotrading/support" style="display: inline-block; background-color: ${accentColor}; color: white; text-decoration: none; padding: 14px 28px; border-radius: 6px; font-size: 14px; font-weight: 500;">
                      Zum Support
                    </a>
                  </td>
                </tr>
    ...
    const { data: emailResult, error: emailError } = await resend.emails.send({
      ...
      subject: `Auftrag: ${payload.task_title} - ${brandingName}`,
      html: htmlEmail,
    });
    ...
    console.log('✅ Task rejected email sent successfully:', emailResult);
    ...
    console.error('💥 Error in send-task-rejected:', error);
    return new Response(
