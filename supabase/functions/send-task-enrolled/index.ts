...
serve(async (req) => {
  ...
  try {
    console.log('📧 Send Task Enrolled email started');

    const payload: TaskEnrolledPayload = await req.json();
...
<head>
  ...
  <title>Geld verdienen freigeschaltet</title>
</head>
<body style="margin: 0; padding: 0; background-color: #f5f5f7; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f5f7;">
          ...
          <tr>
            <td style="padding: 0 40px 40px 40px;">
              ...
              <p style="margin: 0 0 24px 0; color: #4b5563; font-size: 15px; line-height: 1.6;">
                Sie wurden für das Auftrags-Programm freigeschaltet! Ab sofort können Sie Aufträge absolvieren und attraktive Vergütungen verdienen.
              </p>
              ...
              <div style="background-color: #dcfce7; border: 1px solid #86efac; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
                ...
                <ul style="margin: 0; padding-left: 20px; color: #15803d; font-size: 14px; line-height: 1.8;">
                  <li>Flexible Aufträge mit attraktiver Vergütung</li>
                  <li>Sofortige Gutschrift auf Ihr Handelskonto</li>
                  <li>Persönliche Betreuung durch unseren Support</li>
                </ul>
              </div>
              ...
              <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                <tr>
                  <td style="text-align: center;">
                    <a href="https://${domain}/kryptotrading/geld-verdienen" style="display: inline-block; background-color: ${accentColor}; color: white; text-decoration: none; padding: 14px 28px; border-radius: 6px; font-size: 14px; font-weight: 500;">
                      Zu den Aufträgen
                    </a>
                  </td>
                </tr>
    ...
    const { data: emailResult, error: emailError } = await resend.emails.send({
      ...
      subject: `Geld verdienen freigeschaltet - ${brandingName}`,
      html: htmlEmail,
    });
    ...
    console.log('✅ Task enrolled email sent successfully:', emailResult);
    ...
    console.error('💥 Error in send-task-enrolled:', error);
    return new Response(
