...
serve(async (req) => {
  ...
  try {
    console.log('📧 Send KYC Rejection started');

    const payload: KYCRejectionPayload = await req.json();
...
<head>
  ...
  <title>KYC-Verifizierung abgelehnt</title>
</head>
<body style="margin: 0; padding: 0; background-color: #f5f5f7; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f5f7;">
              ...
              <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                <tr>
                  <td style="text-align: center;">
                    <a href="https://${domain}/kryptotrading/kyc" style="display: inline-block; background-color: ${accentColor}; color: white; text-decoration: none; padding: 14px 28px; border-radius: 6px; font-size: 14px; font-weight: 500;">
                      Erneut einreichen
                    </a>
                  </td>
                </tr>
    ...
    const { data: emailResult, error: emailError } = await resend.emails.send({
      ...
      subject: `KYC-Verifizierung abgelehnt - ${brandingName}`,
      html: htmlEmail,
    });
    ...
    console.log('✅ KYC rejection email sent successfully:', emailResult);
    ...
    console.error('💥 Error in send-kyc-rejection:', error);
    return new Response(
