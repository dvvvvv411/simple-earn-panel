...
interface KYCConfirmationPayload {
  user_id: string;
  approved: boolean;
}
...
serve(async (req) => {
  ...
  try {
    console.log('📧 Send KYC Confirmation started');

    const payload: KYCConfirmationPayload = await req.json();
...
<head>
  ...
  <title>KYC-Verifizierung erfolgreich</title>
</head>
<body style="margin: 0; padding: 0; background-color: #f5f5f7; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f5f7;">
          ...
          <tr>
            <td style="padding: 0 40px 40px 40px;">
              ...
              <p style="margin: 0 0 24px 0; color: #4b5563; font-size: 15px; line-height: 1.6;">
                wir freuen uns, Ihnen mitteilen zu können, dass Ihre KYC-Verifizierung erfolgreich abgeschlossen wurde.
              </p>

              <div style="background-color: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
                <p style="margin: 0; color: #065f46; font-size: 15px; line-height: 1.6;">
                  <strong>Sie können nun alle Funktionen unserer Plattform uneingeschränkt nutzen:</strong>
                </p>
    ...
    const { data: emailResult, error: emailError } = await resend.emails.send({
      ...
      subject: `KYC-Verifizierung erfolgreich - ${brandingName}`,
      html: htmlEmail,
    });
    ...
    console.log('✅ KYC confirmation email sent successfully:', emailResult);
    ...
    console.error('💥 Error in send-kyc-confirmation:', error);
    return new Response(
