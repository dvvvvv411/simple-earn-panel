...
interface BankKycConfirmationPayload {
  user_id: string;
...
serve(async (req) => {
  ...
  try {
    console.log('📧 Send Bank KYC Confirmation started');

    const payload: BankKycConfirmationPayload = await req.json();
...
<head>
  ...
  <title>Bank-Verifizierung erfolgreich</title>
</head>
<body style="margin: 0; padding: 0; background-color: #f5f5f7; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f5f7;">
            ...
            <td style="padding: 0 40px 40px 40px;">
              ...
              <div style="background-color: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
                ...
                <table cellspacing="0" cellpadding="0" border="0" width="100%">
                  <tr>
                    <td style="padding: 8px 0; color: #6b7280; font-size: 13px; width: 120px;">Kontoinhaber:</td>
                    <td style="padding: 8px 0; color: #065f46; font-size: 14px; font-weight: 500;">${payload.bank_account_holder || 'N/A'}</td>
                  </tr>
                  <tr>
                    <td style="padding: 8px 0; color: #6b7280; font-size: 13px;">IBAN:</td>
                    <td style="padding: 8px 0; color: #065f46; font-size: 14px; font-weight: 500; font-family: monospace;">${payload.bank_iban || 'N/A'}</td>
                  </tr>
                  <tr>
                    <td style="padding: 8px 0; color: #6b7280; font-size: 13px;">BIC:</td>
                    <td style="padding: 8px 0; color: #065f46; font-size: 14px; font-weight: 500; font-family: monospace;">${payload.bank_bic || 'N/A'}</td>
                  </tr>
                  <tr>
                    <td style="padding: 8px 0; color: #6b7280; font-size: 13px;">Bank:</td>
                    <td style="padding: 8px 0; color: #065f46; font-size: 14px; font-weight: 500;">${payload.bank_name || 'N/A'}</td>
                  </tr>
                </table>
              </div>
              ...
              <div style="background-color: #fef3c7; border: 1px solid #fcd34d; border-radius: 12px; padding: 16px; margin-bottom: 24px;">
                <p style="margin: 0; color: #92400e; font-size: 13px; line-height: 1.6;">
                  <strong>Wichtig:</strong> Bitte verwenden Sie bei Überweisungen als Verwendungszweck Ihre E-Mail-Adresse, damit die Einzahlung Ihrem Konto zugeordnet werden kann.
                </p>
              </div>
    ...
    const { data: emailResult, error: emailError } = await resend.emails.send({
      ...
      subject: `Bank-Verifizierung erfolgreich - ${brandingName}`,
      html: htmlEmail,
    });
    ...
    console.log('✅ Bank KYC confirmation email sent successfully:', emailResult);
    ...
    console.error('💥 Error in send-bank-kyc-confirmation:', error);
    return new Response(
