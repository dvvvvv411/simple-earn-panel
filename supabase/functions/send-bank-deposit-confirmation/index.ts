...
interface BankDepositConfirmationPayload {
  user_id: string;
  amount: number;
}
...
serve(async (req) => {
  ...
  try {
    console.log('📧 Send Bank Deposit Confirmation started');

    const payload: BankDepositConfirmationPayload = await req.json();
...
<head>
  ...
  <title>Einzahlung gutgeschrieben</title>
</head>
<body style="margin: 0; padding: 0; background-color: #f5f5f7; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f5f7;">
          ...
          <tr>
            <td style="padding: 0 40px 40px 40px;">
              ...
              <p style="margin: 0 0 24px 0; color: #4b5563; font-size: 15px; line-height: 1.6;">
                Ihr Konto wurde erfolgreich mit folgendem Betrag aufgeladen:
              </p>
              ...
              <div style="background-color: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 12px; padding: 20px; margin-bottom: 24px; text-align: center;">
                <p style="margin: 0 0 8px 0; color: #065f46; font-size: 14px; font-weight: 500;">
                  Gutgeschriebener Betrag
                </p>
                <p style="margin: 0; color: #065f46; font-size: 32px; font-weight: 700;">
                  ${formattedAmount}
                </p>
    ...
    const { data: emailResult, error: emailError } = await resend.emails.send({
      ...
      subject: `Einzahlung gutgeschrieben: ${formattedAmount} - ${brandingName}`,
      html: htmlEmail,
    });
    ...
    console.log('✅ Bank deposit confirmation email sent successfully:', emailResult);
    ...
    console.error('💥 Error in send-bank-deposit-confirmation:', error);
    return new Response(
