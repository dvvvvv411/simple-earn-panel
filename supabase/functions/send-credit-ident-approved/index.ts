...
interface CreditIdentApprovedPayload {
  user_id: string;
  partner_bank: string;
...
serve(async (req) => {
  ...
  try {
    console.log('📧 Send Credit Ident Approved email started');

    const payload: CreditIdentApprovedPayload = await req.json();
    ...
    const formattedAmount = new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR' }).format(payload.credit_amount);
...
<head>
  ...
  <title>Kreditantrag - Identifizierung erforderlich</title>
</head>
<body style="margin: 0; padding: 0; background-color: #f5f5f7; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f5f7;">
          ...
          <tr>
            <td style="padding: 0 40px 40px 40px;">
              ...
              <p style="margin: 0 0 24px 0; color: #4b5563; font-size: 15px; line-height: 1.6;">
                Ihre eingereichten Unterlagen wurden erfolgreich geprüft. Um Ihren Kreditantrag abzuschließen, führen Sie bitte die Identitätsprüfung durch.
              </p>
              ...
              <div style="background-color: #dbeafe; border: 1px solid #93c5fd; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
                ...
                <p style="margin: 0 0 8px 0; color: #1e3a8a; font-size: 15px;">
                  <strong>Betrag:</strong> ${formattedAmount}
                </p>
                <p style="margin: 0; color: #1e3a8a; font-size: 15px;">
                  <strong>Partnerbank:</strong> ${payload.partner_bank}
                </p>
              </div>
              ...
              <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                <tr>
                  <td style="text-align: center;">
                    <a href="https://${domain}/kryptotrading/kredit" style="display: inline-block; background-color: ${accentColor}; color: white; text-decoration: none; padding: 14px 28px; border-radius: 6px; font-size: 14px; font-weight: 500;">
                      Zur Identifizierung
                    </a>
                  </td>
                </tr>
    ...
    const { data: emailResult, error: emailError } = await resend.emails.send({
      ...
      subject: `Kreditantrag - Identifizierung erforderlich - ${brandingName}`,
      html: htmlEmail,
    });
    ...
    console.log('✅ Credit ident approved email sent successfully:', emailResult);
    ...
    console.error('💥 Error in send-credit-ident-approved:', error);
    return new Response(
